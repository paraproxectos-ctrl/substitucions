...
const Auth: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!loginForm.email.trim()) newErrors.email = t('error.emailRequired');
    if (!loginForm.password) newErrors.password = t('error.passwordRequired');
    setErrors(newErrors);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('Invalid login credentials')) errorMessage = t('error.invalidCredentials');
        else if (error.message.includes('Email not confirmed')) errorMessage = t('error.emailNotConfirmed');
      ...
      <main className="w-full max-w-md" aria-labelledby="auth-title">
        ...
        <Card className="shadow-lg border-0 bg-card/95 backdrop-blur rounded-2xl">
          ...
          <CardContent className="space-y-6 p-6 sm:p-8 pt-0">
            <form onSubmit={handleLogin} className="space-y-4" noValidate autoComplete="on">
              {/* Email */}
